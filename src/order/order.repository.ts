...
import { BaseRepositoryService, BaseTransaction } from 'src/base-repository/base-repository.service';
...
export class OrderRepository extends BaseRepositoryService {
  ...
  async findOrderItems(orderId: number): Promise<OrderItem[]> {
    const orderItems = this.knex.table('order_items')
      .select(['*'])
  ...
  async update(id: number, dto) {
    const order = this.knex.table('orders')
      .update(dto)
      .where({ id })
      .returning('*')
    return order
  ...
  createOrderItem(dto): Promise<OrderItem[]> {
    const orderItem = this.knex.table('order_items')
      .insert(dto)
      .returning('*')
    return orderItem
  }
