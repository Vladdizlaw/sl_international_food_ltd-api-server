...
export class OrderService {
	...
	async createOrder(dto, accountId) {
		...
		const result = await this.OrderRepository.transaction(async (trx) => {
			const [order]: Order[] =
				await this.OrderRepository.transacting(trx).create(orderDto)
			const orderItemsDto = dto.items.map((item) => {
				return {
					...item,
					account_id: accountId,
					order_id: order.id
			...
			const orderItems =
				await this.OrderRepository.transacting(trx).createOrderItem(
					orderItemsDto
				)
